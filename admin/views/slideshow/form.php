...
<div class="panel-body">
    ...
    <form id="slideshowForm" class="form-horizontal" method="post" action="admin.php?controller=slideshow&amp;action=edit" enctype="multipart/form-data" role="form">
        ...
        <div class="form-group">
            ...
            <input class="form-control" type="text" name="title" id="title_slideshow" value="<?php echo $slideshow['title'];   ?>">
        </div>
        <div class="form-group">
            <label class="control-label">Ảnh</label>
            <input name="image" type="file" class="form-control" id="image" <?php if (empty($slideshow)) { echo "required"; } ?> />
            <?php
                if ($slideshow && is_file('public/upload/slideshows/'.$slideshow['image_name'])) {
                    echo '<br><image src="public/upload/slideshows/'.$slideshow['image_name'].'"width="200"/>';
                }
            ?>
        </div>
